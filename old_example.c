...
{
  // Read
  char msg[] = "{test: asdsadsa}";
  int len = strlen(msg);
  char *test = NULL;
  json_scanf(msg, len, "{test: %Q}", &test);
  ...
  len2 = json_printf(&out, "{test: %Q}", msg2);
}
...
static enum MHD_Result save_device_config(const void *cls, const char *mime, struct Session *session, struct MHD_Connection *connection)
{
  ...
  // snprintf (reply, slen + 1, "", session->values[0], session->values[1], session->values[2], session->values[3]);
  // json_scanf(reply, strlen(str), "{ a:%s, b:%s, c:%s }", session->values[0], session->values[1], session->values[2]);
